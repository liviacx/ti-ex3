...
public class ProdutoService {
	...
	public void makeForm(int tipo, Produto produto, int orderBy) {
		...
		if(tipo == FORM_INSERT || tipo == FORM_UPDATE) {
			...
			umProduto += "\t\t</tr>";
			umProduto += "\t\t<tr>";
			umProduto += "\t\t\t<td>&nbsp;Regiao: "+ produto.getRegiao() +"</td>";
			umProduto += "\t\t\t<td>Preco: "+ produto.getPreco() +"</td>";
			umProduto += "\t\t\t<td>Marca: "+ produto.getMarca() +"</td>";
			umProduto += "\t\t</tr>";
			umProduto += "\t\t<tr>";
			umProduto += "\t\t\t<td>&nbsp;Item: "+ produto.getItem().toString() + "</td>";
		...
		} else if (orderBy == FORM_ORDERBY_DESCRICAO) {		produtos = produtoDAO.getOrderByRegiao();
		...
		for (Produto p : produtos) {
			...
			list += "\n<tr bgcolor=\""+ bgcolor +"\">\n" + 
            		  ...
            		  "\t<td align=\"center\" valign=\"middle\"><a href=\"javascript:confirmarDeleteProduto('" + p.getID() + "', '" + p.getRegiao() + "', '" + p.getPreco() + "');\"><img src=\"/image/delete.png\" width=\"20\" height=\"20\"/></a></td>\n" +
            		  "</tr>\n";
		}
		list += "</table>";		
	...
	public Object insert(Request request, Response response) {
		...
		Produto produto = new Produto(-1, regiao, preco, marca, item, dataValidade);
		
		if(produtoDAO.insert(produto) == true) {
            resp = "Produto (" + regiao + ") inserido!";
            response.status(201); // 201 Created
		} else {
			resp = "Produto (" + regiao + ") não inserido!";
